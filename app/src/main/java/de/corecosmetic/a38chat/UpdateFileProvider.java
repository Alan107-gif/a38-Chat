...
public final class UpdateFileProvider extends ContentProvider {
    static final String FILE_NAME = "a38-Chat.apk";
    ...
    public ParcelFileDescriptor openFile(Uri uri, String mode) throws FileNotFoundException {
        if (!"r".equals(mode) || !FILE_NAME.equals(uri.getLastPathSegment())) {
            throw new FileNotFoundException("Update file is unavailable");
        }
        File file = updateFile();
        if (!file.isFile()) {
            throw new FileNotFoundException("Update file is unavailable");
    ...
    private File updateFile() {
        return new File(new File(getContext().getCacheDir(), "updater"), FILE_NAME);
    }
}
